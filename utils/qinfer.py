...
class QueryInfer(object):
    ...
    def _make_sparse_tensor(self, query_logits, last_ys, last_xs, anchors, feature_value):
        ...
        #  进行了修改，将原来的anchors.tensor中的tensor去除，修改最后一位4-》5
        anchors = anchors.view(-1, self.anchor_num, 5)
        selected_anchors = anchors[inds].view(1, -1, 5)
    ...
    def _make_spconv(self, weights, biases):
        ...
        for i in range(len(weights)):
            ...
            # 为了维度对照所修改了，将permute(2, 3, 1, 0)->permute(0, 2, 3, 1)->permute(0, 3, 2, 1)性能高点
            filter.weight.data[:] = weights[i].permute(0, 3, 2, 1).contiguous()[:] # transpose(1,2).transpose(0,1).transpose(2,3).transpose(1,2).transpose(2,3)
    ...
    def run_qinfer(self, model_params, features_value, anchors_value, query_logits):
        ...
        for i in range(len(features_value)-1, -1, -1):
            ...
            bbox_result  = self._run_spconvs(x, self.bbox_spconv).view(-1, self.anchor_num*5)[inds]
