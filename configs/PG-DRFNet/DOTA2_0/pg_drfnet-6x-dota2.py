...
model = dict(
    ...
    bbox_head=dict(
        ...
        guidance_head_size=[192, 192, 4, 1],  # 对应修改前两位置的通道数为输入通道数in_channels
        guidance_layer_train=[1, 2],  # 添加了索引层，找小目标的层
        fp_object_scale=[[0, 32], [0, 64]],  # 添加了小目标范围
        fp_center_dis_coeff=[1., 1.],  # 添加了小目标中心
        guidance_loss_gammas=[1.2, 1.2],  # 添加了损失函数超参数
        guidance_loss_weights=[10., 10.],  # 添加了损失函数超参数
        dynamic_perception_infer=False,  # 是否使用快速推理方法
        dp_version='v1',
        dp_threshold=0.12,
        context=4,
        layers_no_dp=[1, 2, 3],  # 不参与小目标索引的第分辨率特征层p4-p7一共四层
        layers_key_test=[1, 2],  # 特征层的p3,p4作为钥匙来查找p2,p3
        layers_value_test=[0, 1],  # 特征层的p3,p4作为钥匙来查找p2,p3
        loss_guidance_weight=1,  # 为了平横损失差距，对guidance部分损失缩放
