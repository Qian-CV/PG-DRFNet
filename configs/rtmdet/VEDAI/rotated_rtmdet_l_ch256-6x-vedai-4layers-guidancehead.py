...
interval = 4  # 最初是12
angle_version = 'le90'
checkpoint = '/media/ubuntu/nvidia/wlq/part1_tiny_detection/mmrotate-1.x/tools/data/weight/cspnext-l_8xb256-rsb-a1-600e_in1k-6a760974.pth'  # noqa
...
model = dict(
    ...
    bbox_head=dict(
        ...
        query_head_size=[256, 256, 4, 1],  # 对应修改前两位置的通道数为输入通道数in_channels
        query_layer_train=[1, 2],  # 添加了索引层，找小目标的层
        small_object_scale=[[0, 32], [0, 64]],  # 添加了小目标范围
        small_center_dis_coeff=[1., 1.],  # 添加了小目标中心
        query_loss_gammas=[1.2, 1.2],  # 添加了损失函数超参数
        query_loss_weights=[10., 10.],  # 添加了损失函数超参数
        query_infer=False,  # 是否使用快速推理方法
        query_threshold=0.12,
        context=4,
        layers_whole_test=[1, 2, 3],  # 不参与小目标索引的第分辨率特征层p4-p7一共四层
        layers_key_test=[1, 2],  # 特征层的p3,p4作为钥匙来查找p2,p3
        layers_value_test=[0, 1],  # 特征层的p3,p4作为钥匙来查找p2,p3
        loss_query_weight=1,  # 为了平横损失差距，对query部分损失缩放
        # cls_layer_weight=[1.0, 2.6, 1.0, 1.0],  # 分类损失不同层权重占比，防止小目标主导剃度下降
        cls_layer_weight=[1.0, 1.0, 1.0, 1.0],  # 分类损失不同层权重占比，防止小目标主导剃度下降
        # reg_layer_weight=[1.0, 2.6, 1.0, 1.0],  # 回归损失不同层权重占比，防止小目标主导剃度下降
...
work_dir = './work_dirs/vedai/rtmdet/large/rotated_rtmdet_l_ch256-6x-vedai-4layers-guidancehead-accumulate/'
