...
model = dict(
    ...
    bbox_head=dict(
        ...
        small_object_scale=[[0, 64], [0, 128]],  # 添加了小目标范围
        ...
        infer_version='v3',
        context=4,
        layers_whole_test=[2, 3],  # 不参与小目标索引的第分辨率特征层p4-p7一共四层
        layers_key_test=[1, 2],  # 特征层的p3,p4作为钥匙来查找p2,p3
        layers_value_test=[0, 1],  # 特征层的p3,p4作为钥匙来查找p2,p3
        loss_query_weight=1.0,  # 为了平横损失差距，对query部分损失缩放
        cls_layer_weight=[1.0, 1.0, 1.0],  # 分类损失不同层权重占比，防止小目标主导剃度下降
        # cls_layer_weight=[1.0, 2.6, 1.2, 1.0],  # 分类损失不同层权重占比，防止小目标主导剃度下降
        reg_layer_weight=[1.0, 1.0, 1.0],  # 回归损失不同层权重占比，防止小目标主导剃度下降
...
work_dir = './work_dirs/ablation_study/A_study1/test3/A_study-rotated_rtmdet_m_ch192-6x-vedai-3layers-guidancehead1/'
