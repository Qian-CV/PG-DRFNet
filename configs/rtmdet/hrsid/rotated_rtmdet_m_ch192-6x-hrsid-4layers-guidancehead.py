...
model = dict(
    ...
    bbox_head=dict(
        ...
        # cls_layer_weight=[1.0, 1.4, 2.1, 2.5],  # 分类损失不同层权重占比，防止小目标主导剃度下降
        # reg_layer_weight=[1.0, 1.4, 2.1, 2.5],  # 回归损失不同层权重占比，防止小目标主导剃度下降
        cls_layer_weight=[2.4, 4.8, 1.2, 1.0],  # 分类损失不同层权重占比，防止小目标主导剃度下降
        reg_layer_weight=[2.4, 4.8, 1.2, 1.0],  # 回归损失不同层权重占比，防止小目标主导剃度下降
...
work_dir = './work_dirs/hrsid/rotated_rtmdet_m_ch192-6x-hrsid-4layers-guidancehead-[2.4,4.8,1.2,1.0]/'
